function convolve_j, image, psf, FT_PSF=psf_FT, FT_IMAGE=imFT, NO_FT=noft, $
...
;-
	...
	if (sif[0] NE 2) OR (sif[sif[0]+1] NE 6) OR $
	   (sif[1] NE sim[1]) OR (sif[2] NE sim[2]) then imFT = FFT( image,-1,/DOUBLE )

	if keyword_set( auto ) then $
	 return, shift( npix*DOUBLE( FFT( imFT*conj( imFT ),1,/DOUBLE ) ), sc[1],sc[2] )
	...
	if (sp[0] NE 2) OR (sp[sp[0]+1] NE 6) OR $
	   (sp[1] NE sim[1]) OR (sp[2] NE sim[2]) then begin
		...
		L = (s + sim-1) < (sp-1)
		psf_FT = dcomplexarr( sim[1], sim[2] )
		psf_FT[ Loc[1], Loc[2] ] = psf[ s[1]:L[1], s[2]:L[2] ]
		psf_FT = FFT( psf_FT, -1, /OVERWRITE,/DOUBLE )
	   endif

	if keyword_set( correlate ) then $
		conv = npix * DOUBLE( FFT( imFT * conj( psf_FT ), 1,/DOUBLE ) ) $
	  else	conv = npix * DOUBLE( FFT( imFT * psf_FT, 1 ,/DOUBLE) )
